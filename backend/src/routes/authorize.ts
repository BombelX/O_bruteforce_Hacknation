...
router.post("/refresh", async (req, res) => {
  ...
  try {
    resp_db = await db
      .select()
      .from(refresh_tokens)
      .where(eq(refresh_tokens.token, old_refresh_token));
  } catch (error) {
    return res.status(500).json({
      error: "internal server error",
    });
  ...
  const refresh_token = jwt.sign({ _id: resp_db[0].user_id, username: username }, SECRET_KEY, {
    expiresIn: "3d",
  });
  ...
  const token = jwt.sign({ _id: resp_db[0].user_id, username: username }, SECRET_KEY, {
    expiresIn: "1h",
  });
  return res.status(200).json({
